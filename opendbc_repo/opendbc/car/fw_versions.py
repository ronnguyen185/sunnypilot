...
def get_fw_versions_ordered(can_recv: CanRecvCallable, can_send: CanSendCallable, set_obd_multiplexing: ObdCallback, vin: str,
  ...
  # Handle division by zero for VinFast when brand has no ECUs in database (len = 0)
  for brand in sorted(brand_matches, key=lambda b: (brand_matches[b].count(True), 0 if (b == "vinfast" and len(brand_matches[b]) == 0) else brand_matches[b].count(True) / len(brand_matches[b])), reverse=True):
